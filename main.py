...
if __name__ == "__main__":
            ...
            if rule.tags == [] or tag_name[0] in rule.tags:
                ...
                if name:
                    ...
                    add_to_dict(
                        rules,
                        tag_name[0],
                        RuleEntry(tag, name, " ".join(rule.conditions)),
                    )
    ...
    for tag in tag_names:
        ...
        for rules_value in rules_values:
            print(
                "EVENTID={EVENTID}....tag={tag}".format(
                    EVENTID=id, tag=rules_value.tag
                ).ljust(100, "."),
                end="\r",
                flush=True,
            )
    ...
    for dynasty_rule in dynasty_rules:
        ...
        for dynasty_name in dynasty_names:
            print(
                "EVENTID={EVENTID}....dynasty_name={dynasty_name}".format(
                    EVENTID=id, dynasty_name=dynasty_name
                ).ljust(100, "."),
                end="\r",
                flush=True,
            )
    ...
    output_event_file = open(
        "output/events/{event_name}_events.txt".format(event_name=EVENT_NAME), "w+"
    )
    ...
    for rule in rules.items():
        ...
        for name in rule[1]:
            print(
                "tag={tag}".format(tag=name.tag).ljust(100, "."), end="\r", flush=True
            )
            loc += " "
            loc += name.tag
            loc += ': "'
    ...
    for dynasty_rule in dynasty_rules:
        ...
        for dynasty_name in dynasty_names:
            print(
                "dynasty_name={dynasty_name}".format(dynasty_name=dynasty_name).ljust(
                    100, "."
                ),
                end="\r",
                flush=True,
    ...
    localisation_file = open(
        "output/localisation/{event_name}_localisation_l_english.yml".format(
    ...
    on_actions_file = open(
        "output/common/on_actions/{event_name}_on_actions.txt".format(
            event_name=EVENT_NAME
        ),
        "w+",
    )
    ...
    decision_file = open(
        "output/decisions/{event_name}_decisions.txt".format(event_name=EVENT_NAME),
        "w+",
    )
